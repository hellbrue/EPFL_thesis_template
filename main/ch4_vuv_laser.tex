...
This ever growing field has changed in many regards since then, with the introduction of ultrafast spectroscopies being among the more notable ones.
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{(a) Spectrum of Harmonium beamline operated in the \qtyrange{28}{48}{\electronvolt} range. The higher harmonics are generated in Argon. (b) Spectrum of Harmonium with higher harmonics generated in Neon}
...
This beam line is called Harmonium and functions on the principle of higher harmonic generation (HHG) \cite{arrell_harmonium_2017}.
Figure \ref{fig:beamline_cad} shows a top and iso view of the full beamline with specification to the various light sources.
...
After ionization, the now free electron is accelerated in the electric field of the IR driving pulse, gaining kinetic energy.
Some electrons recombine with the ionized atom, at which point they generate an extreme ultraviolet (EUV) emission.
The EUV pulse generated in such a process consists in the frequency domain of a multiple harmonics with an energy of the odd multiple of the seed pulse $(2n+1) \hbar\omega$ and form an harmonic comb.

Mainly two different methods for HHG exist, with the generation process either happening in a gas jet or a gas filled fiber \cite{paul_quasi-phase-matched_2003, heckl_high_2009, sudmeyer_femtosecond_2008, roser_131w220fs_2005}.
In both cases, a comb of harmonics is emitted from the gas with the spectrum of the harmonics depending on phase matching conditions, which can be adjusted to optimize for different energies.
...
The small cross section of the HHG process, due to the low probability of the electron recombining with the ionized gas atom, as wells as the non-perturbative nature of the process are the reason for the need of high peak fluences at high repetition rates, as previously mentioned.
...
Additionally, two torriodal mirrors are used as focusing optics.
The first focuses the harmonics beam onto an adjustable slit, which is located behind the gratings to isolated the selected harmonic.
The single harmonic probe beam is then focused by a second torroidal mirror onto the sample.
...
Part of the amplified IR beam is split of before the HHG process and used to excite the sample.
...
A $\beta$-bariumborate (BBO) crystal can be inserted between the two focusing mirrors to double the pump energy from \qty{1.55}{\electronvolt} to \qty{3.1}{\electronvolt}.

The HHG beamline can produce high flux (flux number) harmonics between \qtyrange{20}{110}{\electronvolt}.
An energy resolution of $\approx$\qty{150}{\milli\electronvolt} can be achieved, which is mainly determined by the chosen harmonic and grating combination as well as vacuum space charge effects, which will be discusse din greater detail in the next section.
...
One of the main limiting factors for the energy resolution in ARPES experiments are vacuum space charge effects.
This effect refers to the fact that a cloud of electrons traveling in vacuum experience a repulsive force between each other due to coulomb interaction.
If a certain density of electrons is reached, the space charge effect becomes strong enough to significantly change the distribution of the electron cloud, which alters the energy, momentum and time information carried by the individual electrons, resulting in a reduced measurement resolutions.
...
The resulting spread due to space charge effects can strongly differ depending on the kinetic energy of the electrons and the time duration of the light pulse generating the electron cloud.
...
Generally speaking, electron bunches resulting from one-photon photoemssion processes in the range of \qtyrange{5}{100}{\electronvolt} and pulse duration in the sub \unit{\pico\second} regime, a linear dependence of the space charge effects to the electron density can be found.
...
The space charge effects are even stronger for the case of pump probe ARPES.
...
Therefore a transition of trARPES experiments, or ARPES in general, to include high repetition rate light sources would also be step in improving the overall resolution of these systems.
...
It is to note here that, while vacuum space charge effects are not the limiting factor in improving the time and energy resolution in the Harmonium beamline, it is still a fundamental physical limitation which provides a further reason to move to high repetition rate systems.
For example, the energy resolution could be improved by better ARPES analyzers, which can provide energy resolutions of up to insert number, instead of the insert number of currently used model. Ask michele
Additionally, the time resolution is rather limited by the pump pulse duration, which could be shortened via pulse compression to the few \unit{\femto\second} regime \cite{nisoli_compression_1997}, or the pulse front tilt introduced by the monochromator.
