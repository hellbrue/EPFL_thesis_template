...
The highest critical temperature of a BCS superconductor can be found in \ce{MgB2} \cite{nagamatsu_superconductivity_2001}.
...
In recent years different approaches have appeared with the goal pushing the critical temperature to room temperature.
...
After that I will first introduce the main observations of the study, in which the quasiparticle peaks in \ce{Bi}2212 shift after light irradiation, creating a metastable state on millisecond timescales.
...
\ce{Bi2Sr2CaCu2O8} (\ce{Bi}2212) is part of the \ce{Bi} based cuprate family.
Similar to all cuprates, they are characterized by \ce{CuO2} planes which are mainly responsible for the electronic properties of the compound.
The cuprates form in perovskite crystal structures, and are typically identified by their amount of \ce{CuO2} planes per unit cell.
Fig. \ref{fig:bscco_structure} shows they crystal structure of the single-, bi- and trilayer \ce{Bi}-based cuprates.
...
In \ce{Bi}2212 such a Lifshitz transition is linked to the existence of a flat band/extended saddle point near ($\pi$,$0$), residing below $E_F$ \cite{campuzano_photoemission_2004,gofron_observation_1994}.
...
The doping for which $T_c$ reaches a maximum is referred to as optimal doping (OP) and for \ce{Bi}2212 a $T_c$ of \qty{90}{\kelvin} can be reached at a doping of 0.16.
...
In this study, we assessed \ce{Bi}2212 single crystals grown at EPFLs crystal growth facility in collaboration with the research group of László Forró.
...
This cut at \qty{-0.423}{\angstrom^{-1}} was then selected to investigate the the out of equilibrium response in the high pump fluence regime.
...
\ce{Bi}2212 is known for the anisotropy between the nodal and anti-nodal regions, which is clearly evident from the d-wave type superconductivty, which results in a superconducting gap at the anti-node but not at the node.
...
To validate the results, the data points of the \qty{0.09}{\milli\joule/\centi\meter\squared} fluence are compared to previously published literature data for optimally doped \ce{Bi}2212 FS.
...
Additional information can be gained from the fluence dependent series of bandmaps.
...
The sides of the \ce{Cu} d-band and the bottom of the parabola are analyzed in similar fashion, validating that the shift is the same across the band strucutre.
...
To answer this question I will use the results on the photodoping and try to recreate the FS within a tight binding approach, which is a standard, but phenomenological method for plotting the FS of \ce{Bi}2212.
...
The tight binding model is fitted to extracted $k_F$ points at the Fermi level.
...
From the analysis of further spectral features, it was possible to observe a rigid band shift, due to a change in the chemical potential.
...
But a first indication that oxygen reordering is relevant for infrared excited \ce{Bi}2212 is given in a recent x-ray photoelectron spectroscopy (XPS) study \cite{puntel_out--equilibrium_2024}.
