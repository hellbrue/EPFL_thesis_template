...
Here, we compare a series of EDCs taken parallel to the K$_2$-$\Gamma$-K$_2$ direction from the $\Gamma$-point to the edge of the 2nd BZ, for both room and low temperature.
...
Another difference between the two phases seems to lie in the flattening of the band dispersion close to $E_F$ at energies above \qty{-0.6}{\electronvolt}.
...
The excitation is created by a \qty{1.55}{\electronvolt} pulse.
...
After a decay of the excitation sets in as expected, but at \qty{500}{\femto\second} a sudden increase in spectral weight occurs in the occupied states.
...
These traces are marked with squared in the band maps, black squares for the unoccupied band structure, white for the occupied region and green for the region with a significant spectral weight after \qty{4}{\pico\second}.
...
Those dynamics already revealed strong oscillation in the intensity of each trace, which will be the subject of this section.
...
With this it is possible to perform a full band mapping of the electron-phonon coupling constant.
...
For this procedure a small binning is applied to the raw data to slightly increase the signal to noise ration.
...
This transform the original 3D data cube from $I(E,k,t)$ to $I(E,k,f)$.
...
This observation agrees as well with the fact, that the CDW is not disrupted immediately by the light pulse, but through the interaction with the phonons, with the the CDW gap being fully quenched after \qty{350}{\femto\second}.
...
While the \qty{1.45}{\tera\hertz} also appears in both LT and HT phase, the band specific electron-phonon coupling changes quite significantly, especially in the center of the of BZ close to $\Gamma$ and in the places of minigap formation.
Here appears the \qty{0.4}{\tera\hertz} mode at LT, coupling to the bands that are affected by the CDW gap formation.
...
The orbital which is most affected by the structural phase transition is the $d_{xy}$ orbital, it is possible that the changed overlap due to the heptamere formation effects the electron-phonon coupling.
...
Recently two groups have reported on a metastable state, observed in pump probe electron diffraction measurements \cite{siddiqui_ultrafast_2021, domrose_femtosecond_2024}.
...
Therefore I will expand on these reports in this section, providing information on the electronic structure at long delays after pump excitation and the various dynamics occurring at different timescales.
...
This time the range of delay steps was increased heavily, measuring with a relatively fine step size of \qty{250}{\femto\second} from before $t_0$ to \qty{5}{\pico\second}, followed by progressively increased step sizes up to \qty{200}{\pico\second} after pulse excitation.
...
The new electronic structure is stable beyond \qty{200}{\pico\second} after excitation.
...
Here after the initial excitation, the population decay and stabilized after \qty{10}{\pico\second} and barely changes in intensity until \qty{200}{\pico\second} after pump excitation.
...
While in the LT phase a similar relaxation timescale can be observed, the band does not relax back to the equilibrium energy position, but rather overshoots to a binding energy \qty{30}{\milli\electronvolt} below its equilibrium.
...
Perturbing the LT ground state of the material clearly does not only lead to an excitation of the electron bath, but instead launches many strong phonon, either impulsively or through the strong electron-phonon coupling.
...
It underlines the cooperative character the charge order and structure have on each other, when establishing the ground state.
...
The spectral weight further signifies the new metastable equilibrium that has been reached.
...
The persisting change in the core level binding energy of the \ce{Te} 4d orbital, also reflects this change, although the magnitude of the shifted core level is not the same as the increase of $\mu$ at $E_F$.
...
While the left difference map, representing a low fluence, shows to bands dispersing to the left side without any kinks, the two other maps show these bands bending to the edge in the marked area.
This kinked structure looks more like the metastable bandmaps plotted in \ref{fig:tateltmetasteps}, whereas the bandmap on the left rather looks the equilibrium band structure albeit with an elevated population compared to equilibrium.
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.33\textwidth}
		...
		\caption{}
	\end{subfigure}
	\caption{(a) Difference maps between before $t_0$ and \qty{20}{\pico\second} for three selected fluences. The region of the black rectangle shows a kink in the band, that has not decayed within \qty{20}{\pico\second} and represents the metastable band reordering. (b) Integrated intensity of the black rectangle region in (a). A sharp threshold fluence, needed to reach a persistent metastable state beyond \qty{20}{\pico\second}, is observed.}
	\label{fig:tate_fluence}
\end{figure}

Following the evolution of the integrated area within the black rectangle shows a clear threshold fluence at which the band structure changes from the straight to the kinked band.
This evolution can be observed in Fig. \ref{fig:tate_fluence}.
A fluence of 9\% shows a strong increase in intensity for the marked area, corresponding to the appearance of the kink.
At higher fluences the kink structure persists but charge effects seem to play a role, which can be identified by rigid band shift to higher energies.
...
The fact that a cutoff fluence shows in this graph does not necessarily mean that it is a minimum fluence to enter this metastable state.
...
Additionally they show, that it is a highly nonlinear process, as would be expected from a process that relies on modification of the free energy landscape.

\section{Conclusion and Outlook}

After analyzing the various electronic signatures, their dynamics and couplings in detail it is important to overall physics at play.
While each of these items have been separated into individual sections, it is important to remember that they can't be discussed as isolated entities in a vacuum.
...
In both phases the Fermi surfaces shows a strong 1D character with two Fermi surface sheets that could fulfill nesting conditions.
...
A quench of the CDW gaps was observed, but it could be shown that the quench was a delayed reaction to the laser excitation.
Typically the standard way of thinking is that the electromagnetic field couples directly to the charge order, disrupting it and ultimately disturbing the CDW.
While this might be the case for other materials where a direct response is observed, the delayed quench in \ce{TaTe2} demonstrates that the charge order is quite resilient against intense femtosecond light illumination and the dominant contribution to the disruption comes from lattice vibrations.

These lattice vibrations are either launched impulsively or through the strong electron-phonon coupling, or a combination.
Either way it is important to think about the phononic contribution being relevant at the earliest time intervals after excitation and certainly not only setting in after few hundreds of \unit{\femto\second} as is often the case in literature.
...
I already argued that vibrational modes are exited on ultrafast timescales, directly impacting the crystal structure.
Within \qty{450}{\femto\second} the CDW is fully quenched further influencing the destabilized system.
...
A disturbed structure and a perturbed charge order at low temperature seem to lead to the occupation of this local minima with the emergence of the metastable state.
